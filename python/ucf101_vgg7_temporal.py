...
class UCF101VGG7_Temporal(CNN_accelerator):
    def __init__(self, config, layers, params_path = None):
        super(UCF101VGG7_Temporal, self).__init__(config)
        self.layers = layers
...
def ucf101_vgg7_temporal(model_path, config, accelerator):
    layers = make_layers(config, in_channel=20, accelerator=accelerator)
    params_path = model_path
    model = UCF101VGG7_Temporal(config, layers, params_path = params_path)
    return model        
